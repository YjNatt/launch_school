...
class TodoListTest < MiniTest::Test
  ...
  def test_add
    new_todo = Todo.new("Something else")
    @list.add(new_todo)
    @todos << new_todo
    assert_equal(@todos, @list.to_a)
  end
  ...
  def test_item_at
    assert_raises(IndexError) { @list.item_at(34)}
  ...
  def test_select
    list_done_todos = @list.select { |todo| todo.done? }
    assert_instance_of(TodoList, @list.select { nil })
    assert_equal([], list_done_todos.to_a)
